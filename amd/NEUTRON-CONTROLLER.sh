...
openstack endpoint create --region RegionOne network public http://${CON_IP}:9696

openstack endpoint create --region RegionOne network internal http://${CON_IP}:9696

openstack endpoint create --region RegionOne network admin http://${CON_IP}:9696
...
crudini --set /etc/neutron/neutron.conf database connection mysql+pymysql://neutron:${STACK_PASSWD}@${CON_IP}/neutron
...
crudini --set /etc/neutron/neutron.conf nova auth_url http://${CON_IP}:5000
...
crudini --set /etc/neutron/metadata_agent.ini DEFAULT nova_metadata_host ${CON_IP}
crudini --set /etc/neutron/metadata_agent.ini DEFAULT metadata_proxy_shared_secret ${STACK_PASSWD}

crudini --set /etc/nova/nova.conf neutron url http://${CON_IP}:9696 
